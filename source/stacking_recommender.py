...
class StackingModel(AlgoBase):
    ...
    def __init__(self, train_data):
        ...
        self.model_selection = [['baselineonly', BaselineOnly(bsl_options={'method': 'als',
                                                              'n_epochs': 25, 'reg_u': 5, 'reg_i': 3})],
    ...
    def fit(self, train_data, retrain=True, retrain_split_num=2):
        ...
        if retrain:
            ...
            for model_name, model in self.model_selection:
                ...
                for trainset, testset in kSplit.split(train_data):
                    ...
                    self.model_rmse[model_name] = accuracy.rmse(model_prediction, verbose=True)
        ...
        for model in self.model_list:
            ...
            rating = pd.DataFrame(pred, columns=['uid',
                                                 'iid',
                                                 'rating',
                                                 f'predicted_rating_{model}',
                                                 'details'])[['rating']]
            pred_df = pd.DataFrame(pred, columns=['uid',
                                                  'iid',
                                                  'rating',
                                                  f'predicted_rating_{model}',
                                                  'details'])[[f'predicted_rating_{model}']]
        ...
        reg = LinearRegression(fit_intercept=False).fit(prediction_stacking.iloc[:, 1:],
                                                        prediction_stacking.iloc[:, 0])
        self.weights = np.array(reg.coef_)

    def estimate(self, u, i):
        if (self.trainset.knows_user(u) and self.trainset.knows_item(i)):
            algoResults = np.array([self.model_list[model].predict(self.trainset.to_raw_uid(u),
                                                                   self.trainset.to_raw_iid(i))[3]
                                    for model in self.model_list])
